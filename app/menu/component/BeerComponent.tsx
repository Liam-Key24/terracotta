...
import { beersData } from "../../data/beerData";
    ...
    <div className="w-auto h-auto flex flex-col md:flex-row md:space-x-10 space-y-6 md:space-y-0">
      ...
      <div className="relative inset-0 md:w-1/2">
        <div className="absolute inset-0 z-30 flex justify-center items-center">
          <h2 className="text-5xl text-white/80">Beers</h2>
        </div>

        <div className="relative h-50 md:h-[20vh] inset-0 bg-[url(/assets/menu/Beer-image.jpg)] bg-cover bg-center rounded-lg"></div>
        <div className="absolute h-50 md:h-[20vh] inset-0 bg-black/30 backdrop-blur-xs rounded-lg"></div>
      </div>

      {/* RIGHT SIDE (MENU ITEMS) */}
      <div className="grid grid-cols-1 gap-4 md:w-1/2 place-content-center">
        {beersData.map((item, index) => (
